...
const Clientes = () => {
  ...
  const handleClientsData = useCallback(async () => {
    try {
      const { data, response = null } = await findClients(token)
      if (response) {
        const { data } = response
        const { message } = data
  ...
  const updateClientList = (id) => {
    setClients((prevState) => {
      return prevState.filter((client) => client._id !== id)
    })
  }
  ...
  return (
    <div className={AddAnimClass('fadeInRight')}>
      ...
      <Link className="btn btn-verde nvo-cliente" to={'/clientes/nuevo'}>
