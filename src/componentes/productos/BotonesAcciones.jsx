...
import { saveImageData, deleteImageFromCloudinary } from './handleProductImage'
...
const BotonesAcciones = (props) => {
  ...
  const { token, imagedata, idProduct } = props
  ...
  const saveImage = async () => {
    try {
      ...
      const { data, response = null } = await saveImageData(
        idProduct,
        imageMetadata,
        token
      )
  ...
  const cancelImage = async () => {
    try {
      ...
      const { data, response = null } = await deleteImageFromCloudinary(
        public_id,
        token
      )
      if (response) {
  ...
  return (
    <div className={AddAnimClass('fadeInUp')}>
      <button type="button" className="btn btn-verde" onClick={saveImage}>
        Guardar
      </button>
      <button type="button" className="btn btn-rojo" onClick={cancelImage}>
        Cancelar
      </button>
    </div>
  )
