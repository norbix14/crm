  ...
  const loadImage = async () => {
    try {
      if (Object.keys(credentials).length <= 0) {
        return Toast('warning', 'Intente más tarde debido a un error')
      }
      const { url, key, signature, time_stamp, public_id } = credentials
      ...
      const { data, response = null } = await uploadToCloudinary(url, formData)
      if (response) {
        const { data } = response
        const { message } = data
      ...
      return Toast('error', 'Ha ocurrido un error al cargar la imagen')
    }
  }

  return (
    <div className={AddAnimClass('fadeInUp')}>
      <button type="button" className="btn btn-azul" onClick={loadImage}>
        Cargar
      </button>
      <progress
